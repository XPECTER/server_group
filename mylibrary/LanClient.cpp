...
bool CLanClient::Connect(wchar_t *szClientBindIP, wchar_t *szServerIP, int iServerPort, int iThreadNum, bool bEnableNagle)
{
	...
	if (INVALID_SOCKET == this->_ClientSock)
	{
		...
		InetPton(AF_INET, szClientBindIP, &this->_ServerAddr.sin_addr);
		...
		this->_ServerAddr.sin_port = htons(iServerPort);
		InetPton(AF_INET, szServerIP, &this->_ServerAddr.sin_addr);

		if (true == bEnableNagle)
		{
			if (0 != setsockopt(this->_ClientSock, IPPROTO_TCP, TCP_NODELAY, (char *)true, sizeof(bool)))
			{
	...
	if (NULL == this->_hWorkerThread)
	{
		this->_hWorkerThread = new HANDLE[iThreadNum];
